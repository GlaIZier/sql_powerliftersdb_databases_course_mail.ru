...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`city` (
  ...
  PRIMARY KEY (`city_id`),
  UNIQUE INDEX `city_UNIQUE` (`city_name` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`country` (
  ...
  UNIQUE INDEX `country_UNIQUE` (`country_name` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`powerlifter` (
  ...
  INDEX `fk_powerlifter_city2_idx` (`city_id` ASC),
  INDEX `fk_powerlifter_country1_idx` (`country_id` ASC),
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`competition` (
  ...
  INDEX `fk_competition_federation1_idx` (`federation_id` ASC),
  INDEX `fk_competition_country1_idx` (`country_id` ASC),
  INDEX `fk_competition_city1_idx` (`city_id` ASC),
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`age_class` (
  ...
  UNIQUE INDEX `age_class_UNIQUE` (`age_class_name` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`competition_result` (
  ...
  INDEX `fk_powerlifter_has_competition_competition1_idx` (`competition_id` ASC),
  INDEX `fk_powerlifter_has_competition_powerlifter_idx` (`powerlifter_id` ASC),
  INDEX `fk_competition_result_weight_class1_idx` (`weight_class_id` ASC),
  INDEX `fk_competition_result_age_class1_idx` (`age_class_id` ASC),
...
CREATE TABLE IF NOT EXISTS `powerliftersdb`.`exercise_result` (
  ...
  INDEX `fk_exercise_result_competition_result1_idx` (`competition_result_id` ASC),
  INDEX `fk_exercise_result_exersice1_idx` (`exersice_id` ASC),
